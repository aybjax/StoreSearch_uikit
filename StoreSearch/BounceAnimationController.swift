    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
           ...
           let toView = transitionContext.view(forKey: UITransitionContextViewKey.to)
        {
            ...
            UIView.animateKeyframes(withDuration: transitionDuration(using: transitionContext), delay: 0, options: .calculationModeCubic, animations: {
                UIView.addKeyframe(withRelativeStartTime: 0.0, relativeDuration: 0.344, animations: {
                    toView.transform = CGAffineTransform(scaleX: 1.2, y: 1.0)
                })
                UIView.addKeyframe(withRelativeStartTime: 0.344, relativeDuration: 0.333, animations: {
